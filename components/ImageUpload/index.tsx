...
const ImageUpload = () => {
  ...
  const uploadImage = async () => {
    ...
    data.append('folder', process.env.NEXT_PUBLIC_CLOUDINARY_FOLDER)
    // for (const pair of data?.entries()) {
    // console.log('--uploadImage data: ' + pair[0] + ', ' + pair[1])
    // }
    try {
          ...
          {preview && (
            <>
              <div className="flex justify-end pb-8 pt-6 gap-4">
                <button
                  onClick={uploadImage}
                  className="pointer z-10 mt-6 rounded-lg border border-blue-700 bg-blue-700 text-white px-3 py-2 text-sm font-semibold text-black transition hover:bg-white/10 hover:text-white md:mt-4 disabled:opacity-25"
                ...
                <button
                  ...
                  className="pointer z-10 mt-6 rounded-lg border border-red-700 bg-red-700 hover:bg-red-500 text-white px-3 py-2 text-sm font-semibold text-black transition hover:bg-white/10 hover:text-white md:mt-4 disabled:opacity-25"
                  disabled={loading}
                >
                  Reset
                </button>
